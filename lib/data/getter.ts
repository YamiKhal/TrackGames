...
import { fetchAPI } from "../igdb/IGDBAPI";
...
export async function getByIds<T>(ids: number[], select: {}, database: PrismaClient, fetching: {endpoint: string, body: string}, formatter: (data: any) => T): Promise<T[]> {
    ...
    return ids
        ...
        .filter((game): game is T => Boolean(game));
}


export async function getBySlugs<T>(slugs: string[], select: {}, database: PrismaClient, fetching: {endpoint: string, body: string}, formatter: (data: any) => T): Promise<T[]> {
