...
export default function SubTabs({ tabs, active, setter, viewAll, compact, children }: { tabs: { id: string; label: string; }[]; active: any; setter: Dispatch<any> | Function; viewAll?: boolean; viewAllHref?: string; compact?: boolean; children?: ReactNode }) {
    if (!tabs.length) {
        return null;
    }
            ...
            {compact && (
                <div className="mb-5 flex min-w-0 items-center gap-2 md:hidden">
                        ...
                        {tabs.map((tab) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`min-w-0 max-w-28 shrink-0 rounded px-2 py-2 text-xs font-bold transition ${isActive ? "bg-primary text-text-inverse" : "bg-bg-secondary/50 border border-border text-text-muted hover:text-text"}`}
                    ...
                    {viewAll && (
                        <button
                            type="button"
                            className="group cursor-pointer flex shrink-0 items-center border-b border-border px-2 py-2 text-xs font-bold text-text-muted transition-colors hover:text-text"
                        >
                            View all
                        </button>
                    )}
