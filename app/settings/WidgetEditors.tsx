	...
	return (
		<div className="flex flex-col gap-3 pb-3 md:flex-row md:items-center md:justify-between">
			<button
				...
				className="cursor-pointer flex flex-row gap-2 items-center rounded p-2 text-text-muted hover:text-primary"
			...
			{selectedStats.length === 0 ? (
				...
				<div className="grid gap-3 sm:grid-cols-[repeat(auto-fit,minmax(9rem,1fr))]">
					{selectedStats.map((stat, index) => (
						<div
							key={stat}
							className="flex min-h-28 min-w-0 flex-col justify-center items-center gap-3 rounded border border-primary/80 bg-primary/5 p-3"
						>
							<p className="mt-2 truncate text-2xl font-semibold leading-tight text-text">000</p>
							<p className="mt-2 truncate text-md font-semibold leading-tight text-text-muted">
								{normalize.label(statOptions, "value", "label", stat, stat)}
							</p>
				...
				<div className="mt-2 flex flex-col gap-3">
					<div className="relative">
						...
						<Input
							...
							placeholder="Add game from your library"
							className="pl-9 pr-9"
						/>
						{query && (
							<button
	...
	return (
		<div className="border-b border-border p-3 pb-6">
			...
			{widget.type === WidgetType.MARKDOWN && (
				<MarkdownWidgetEditor value={widget.content} onChange={(content) => onChange({ content })} />
			)}
		</div>
	);
}
